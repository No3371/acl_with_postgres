CREATE OR REPLACE FUNCTION pgtap_test_perm_tree(
) RETURNS SETOF TEXT AS $$
BEGIN
    INSERT INTO user DEFAULT VALUES;
    INSERT INTO role (role_name, priority) VALUES ('admin', 100), ('moderator', 50), ('default', 0);
    INSERT INTO role (role_name, priority) VALUES ('market_owner_', 100), ('moderator', 50), ('default', 0);
...
CREATE OR REPLACE FUNCTION pgtap_test_perm_value(
) RETURNS SETOF TEXT AS $$
BEGIN
    INSERT INTO user DEFAULT VALUES;
    INSERT INTO role (role_name, priority) VALUES ('admin', 100), ('moderator', 50), ('default', 0);
    ...
    INSERT INTO role_perm (role_id, perm_id, scope, state) VALUES
        ((SELECT get_role_id('admin')), (SELECT get_perm_id('market')), 0, true);

    INSERT INTO user_role (user_id, role_id) VALUES (1, (SELECT get_role_id('admin')));
...
CREATE OR REPLACE FUNCTION pgtap_test_cache(
) RETURNS SETOF TEXT AS $$
BEGIN
    INSERT INTO user DEFAULT VALUES;
    INSERT INTO role (role_name, priority) VALUES ('admin', 100), ('moderator', 50), ('default', 0);
    ...
    RETURN NEXT is((SELECT COUNT(*) FROM permission_cache), 4, 'Should have cached 4 perms');

    DELETE FROM user_role WHERE user_id = 1;

    RETURN NEXT is((SELECT COUNT(*) FROM permission_cache), 0, 'Should have cleared cache');

    RETURN NEXT is((SELECT check_permission_cached(1, 'market/create', 0)), true, 'market/create: true (cache)');
    
    RETURN NEXT is((SELECT COUNT(*) FROM permission_cache), 1, 'Should have cached 1 perm');
